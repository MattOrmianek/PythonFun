...
class GatewayService(ZMQ):
    ...
    def _on_message(
    ...
    ):  # TODO: adjust the parser to new format / check if current one works as well
        values = msg.topic.split(
            "/"
        )  # retrieve topic name without id // TODO: slice better
        data = {}
        try:
        ...
        # Log minified string
        self._log(
            f'Received MQTT {values[1]} mesage: {json.dumps(data, separators=(",", ":"))}'
        )
    ...
    def _parse_msg(self, topic, data):
        if topic == "request":
            if "type" not in data:
                ...
                if "gpio" in data:
                    self._send("gpio", "request", json.dumps(data["gpio"]["data"]))
            ...
            if data["type"] == "network_config":
                if "wifi" in data:
                    if "STA" in data["wifi"]:
                        ...
                        self._send(
                            "network", "config_STA", json.dumps(data["wifi"]["STA"])
                        )
                    if "AP" in data["wifi"]:
                        self._log("Received AP configuration")
                        self._send(
                            "network", "config_AP", json.dumps(data["wifi"]["AP"])
                        )
                if "ethernet" in data:
                    self._send(
                        "network", "config_ethernet", json.dumps(data["ethernet"])
                    )
                ...
                if "url" in data:
                    ...
                    config_file_path = (
                        "/root/testing/config.json"  #! change it after tests
                    )
                    ...
                    try:
                        ...
                        args = [
                            ...
                            location,
                            url,
                        ]  # remove '-q' parametr for log output
                        output = subprocess.Popen(args)
                    ...
                    time.sleep(
                        1
                    )  # time.sleep for waiting to save downloaded file, don't remove it
                ...
                if test == True:
                    ...
                    # Saving old settings
                    with open(
                        "/root/baseboard/services/gateway/config/mqtt_config.json", "r"
                    ) as f:
                        mqtt_config = f.read()
                        f.close()
                    ...
                    self.client.on_disconnect = self._on_disconnect
                    self.client.on_message = self._on_message
                    self.client.username_pw_set(new_login, new_password)
                    try:
                        self._log("Trying to connect with new settings")
                        if self.client.connect(new_host, new_port) == True:
                            with open(
                                "/root/baseboard/services/gateway/config/mqtt_config.json",
                                "w",
                            ) as file:
                        ...
                        self._log(
                            "Connection refused, can't connect, backing up to old settings."
                        )
                        with open(
                            "/root/baseboard/services/gateway/config/mqtt_config.json",
                            "w",
                            ...
                            self.client.username_pw_set(
                                mqtt_config["login"], mqtt_config["password"]
                            )
                            try:
                                #! bez _async
                ...
                list_of_paths = [
                    "/root/baseboard/services/can/config/can_config",
                    "/root/baseboard/services/can/config/service_config",
                    "/root/baseboard/services/gateway/config/mqtt_config",
                    "/root/baseboard/services/gateway/config/service_config",
                    "/root/baseboard/services/gpio/config/gpio_config",
                    "/root/baseboard/services/gpio/config/service_config",
                    "/root/baseboard/services/led/config/service_config",
                    "/root/baseboard/services/lte_monitor/config/lte_monitor_config",
                    "/root/baseboard/services/lte_monitor/config/service_config",
                    "/root/baseboard/services/network/config/service_config",
                    "/root/baseboard/services/touch/config/mqtt_config",
                    "/root/baseboard/services/touch/config/service_config",
                    "/root/baseboard/services/vpn/config/service_config",
                ]
        ...
        # removing errors from devices
            ...
            with open(
                "/root/baseboard/services/vpn/config/cert.ovpn", "w"
            ) as ovpn_file:
    ...
    def _gpio_config_event(self, data):
        ...
        data = json.loads(data)
        frame = {"type": "gpio_config", "timestamp": int(time.time()), "gpio": data}
    ...
    def _can_config_event(self, data):
        ...
        data = json.loads(data)
        frame = {"type": "pm_config", "timestamp": int(time.time()), "pm": data}
        self.client.publish(f"/maintenance/status/{self.id}", json.dumps(frame))
        # self._pub_full_status()
    ...
    def _no_main_power_warning(self):
        ...
        frame = {
            "type": "warning",
            "timestamp": int(time.time()),
            "details": {"code": 999, "description": "Main power loss"},
        }
    ...
    def _anti_theft_active(self):
        ...
        frame = {
            "type": "warning",
            "timestamp": int(time.time()),
            "details": {"code": 997, "description": "Anti-theft button active"},
        }
        self.client.publish(f"/maintenance/warning/{self.id}", json.dumps(frame))
    ...
    async def _heartbeat(self):
        ...
        while True:
            ...
            hb = {"type": "hb", "timestamp": int(time.time()), "devices": device_ids}
